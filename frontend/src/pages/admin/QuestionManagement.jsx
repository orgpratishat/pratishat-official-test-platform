...
//     <div
//       key={i}
//       className={`mb-4 p-4 border rounded-lg transition-all duration-200 ${
//         opt.isCorrect
...
//         {/* Text editor for the option text */}
//         <div className="flex-1">
//           <RichTextEditor
//             value={opt.optionText}
//             onChange={(value) => toggleOption(i, 'optionText', value)}
...
//       // Extract chapters from all questions (now chapters is an array)
//       const physicsChapters = [...new Set(physicsQuestions.flatMap(q => q.chapters).filter(Boolean))].sort();
//       const physicsTopics = [...new Set(physicsQuestions.flatMap(q => q.topics).filter(Boolean))].sort();
      
//       const chemistryChapters = [...new Set(chemistryQuestions.flatMap(q => q.chapters).filter(Boolean))].sort();
//       const chemistryTopics = [...new Set(chemistryQuestions.flatMap(q => q.topics).filter(Boolean))].sort();
      
//       const biologyChapters = [...new Set(biologyQuestions.flatMap(q => q.chapters).filter(Boolean))].sort();
//       const biologyTopics = [...new Set(biologyQuestions.flatMap(q => q.topics).filter(Boolean))].sort();
...
//   // Enhanced Fuse search to include chapters and createdBy information
//   const fuse = useMemo(() => new Fuse(questions, {
//     keys: [
//       'questionText',
//       'subject',
...
//   const getSubjectSpecificOptions = () => {
//     const currentData = newMode ? newData : formData;
//     const selectedSubject = currentData.subject;
...
//   // ... (rest of the file upload and image handling functions remain the same)
...
//       ...initialFormData,
//       ...q,
//       // Ensure chapters is an array (handle both old single chapter and new multiple chapters)
//       chapters: Array.isArray(q.chapters) ? q.chapters : (q.chapter ? [q.chapter] : []),
//       options: q.options?.length ? q.options : Array(4).fill({ optionText: '', optionImage: '', isCorrect: false }),
//       hint: q.hint || { text: '', image: '' },
//       approach: q.approach || { text: '', image: '' },
//       solution: q.solution?.length ? q.solution : [{ stepNumber: 1, stepText: '', stepImage: '' }],
